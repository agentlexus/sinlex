...
| 2026-06-22 | **Сложность мелких деталей:** `_classify_complexity_hint` — для V<8000 мм³ sa/v не используется; `detail_index` + cap `hex_head_stud` → «низкая». Крупные детали — прежние пороги sa/v. Тест `test_complexity_classify`. |
