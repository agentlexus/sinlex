"""Приём ответов IMAP: сопоставление reply по Message-ID."""
...
LOG = logging.getLogger("email_logistics.imap")
...
def strip_quoted_reply(text: str) -> str:
    ...
    for line in lines:
        ...
        if "-----original message-----" in low:
            break
        if line.strip().startswith("From:") and out:
            break
        out.append(line)
    return "\n".join(out).strip()
...
def _match_task_id(
    ...
    if HYBRID_REPLY_TAG in body or HYBRID_TAG in body:
        for tid in pending:
            if tid in body:
                return tid
...
def _is_outbound_service_body(body: str) -> bool:
    low = (body or "").strip()
    if low.startswith(HYBRID_TAG) and "task_id:" in low.lower():
        return True
    if low.startswith(HYBRID_BALANCE_TAG) and "task_id:" in low.lower():
        return True
    return False
    ...
    for num in data[0].split():
        ...
        if processed_folder:
            try:
                ...
                LOG.warning("imap copy to %s failed", processed_folder)


def poll_balance_inbox(cfg: dict, state: Dict[str, Any]) -> None:
    """IMAP: ответы на служебные письма → state['balance_responses']."""
    ...
    try:
        ...
        def on_match(tid: str, body: str, num: str) -> None:
            ...
            if tokens is None:
                LOG.warning("balance reply unparsed task_id=%s", tid)
                return
            ...
            LOG.info("email_logistics balance response task_id=%s tokens=%s", tid, tokens)
...
def poll_inbox(cfg: dict, state: Dict[str, Any]) -> None:
    """Один проход IMAP: заполняет state['responses'], снимает matched из pending."""
    ...
    try:
        ...
        def on_match(tid: str, body: str, num: str) -> None:
            ...
            LOG.info(
                "email_logistics response task_id=%s chars=%s",
                tid,
                len(body),
            )
